...
unless file_loaded?(__FILE__)
	# Define a toolbar
	barraet = UI::Toolbar.new("Importar Etiquetas")

	# Executa o módulo do botão de edição de etiquetas
	edet = UI::Command.new("Edita Etiquetas") do
	...
	edet.small_icon = edet.large_icon = File.join(File.dirname(__FILE__).gsub('\\', '/'), "ImpTag/Img/edita.png")
	edet.tooltip = "Edita Etiquetas"
	barraet.add_item edet

	# Executa o módulo do botão de geração de etiquetas
	cret = UI::Command.new("Cria Etiquetas") do
	...
	cret.small_icon = cret.large_icon = File.join(File.dirname(__FILE__).gsub('\\', '/'), "ImpTag/Img/cria.png")
	cret.tooltip = "Cria Etiquetas"
